...
@Repository
public class MenuItemDaoImpl2 extends BaseDaoImpl<MenuItem2, String> implements
		MenuItemDao2 {

	@Override
	protected Class<MenuItem2> getEntityClass() {
		return MenuItem2.class;
	}
	...
	*/
	public List<MenuItem2> findTopMenu() {
		return (List<MenuItem2>) this.queryByHql("from MenuItem where deleted=false and father.id is null order by priority ASC",
				null);
	}
	...
	public List<MenuItem2> findByParentId(String id) {		
		...
		return (List<MenuItem2>) this.queryByHql(hql,
				null);
	}
	...
	public String menuToJson() {
		// TODO Auto-generated method stub
		List<MenuItem2> menuTop = findTopMenu();
		List<MenuItem2> menuAllChild = findAllChild();
		//表示通过数据库查询出来的顶级菜单为空
		if(menuTop.size()==0)
			return "{\"state\":\"empty\"}";
		else
	 ...
	 * @return
	 */
	public String oneMenuToJson(MenuItem2 temp ,List<MenuItem2> menuAllChild) 
	{
		  List<MenuItem2> tempChild = new ArrayList<MenuItem2>();
      ...
      {  
    	  ...
    	  for(MenuItem2 subTemp:tempChild)
    	  {	
	...
	@SuppressWarnings("unchecked")
	@Override
	public List<MenuItem2> findAllChild() {	
