...
<title>饼图专题图</title>
<link rel="stylesheet"
	href="/arcgis/library/3.9/3.9/js/dojo/dijit/themes/claro/claro.css" />
<link rel="stylesheet"
	href="/arcgis/library/3.9/3.9/js/esri/css/esri.css">
<link href="../js/arcgis/css/ChartInfoWindow.css" rel="stylesheet" />
<style>
html, body, #map {
	height: 100%;
	margin: 0;
...
<script src="/arcgis/library/3.9/3.9/init.js"></script>
<script>
	require(
			[ "esri/map", "esri/layers/FeatureLayer",
					...
					"myDojo/ChartInfoWindow",
					"myDojo/CustomTheme", "myDojo/geometryUtils",
					"dojo/_base/array", "dojo/dom-construct",
					"dojo/_base/window", "dojox/charting/Chart",
					"dojox/charting/action2d/Highlight",
					"dojox/charting/action2d/Tooltip",
					"dojox/charting/plot2d/Pie", "dojo/domReady!" ],
			function(Map, FeatureLayer, ArcGISTiledMapServiceLayer,
					SimpleLineSymbol, SimpleFillSymbol, SimpleRenderer, Color,
					ChartInfoWindow, CustomTheme, geometryUtils, array,
					domConstruct, win, Chart, Highlight, Tooltip) {
				var map = new Map("map", {
				...
				var featureLayer = new FeatureLayer(
						"http://sampleserver1.arcgisonline.com/ArcGIS/rest/services/Specialty/ESRI_StateCityHighway_USA/MapServer/1",
						{
							...
							outFields : [ "STATE_NAME", "WHITE", "BLACK",
									"AMERI_ES", "ASIAN_PI", "OTHER" ]
						});
				...
				featureLayer.on("update-end", function(evt) {
					var showFields = [ "WHITE", "BLACK", "AMERI_ES",
							"ASIAN_PI", "OTHER" ];
					...
					array
							.forEach(
									...
									function(graphic, index) {
										...
										var styleStr = "width:" + radius
												+ "px;height:" + radius + "px";
										var nodeChart = domConstruct.create(
												"div", {
													id : 'nodeTest' + index,
													style : styleStr
												}, win.body());
										var chart = makePieChart(
												nodeChart,
												layer.graphics[index].attributes,
												showFields);
										var optinal = true;
										...
										infoWindow.resize(radius + 2,
												radius + 2);
										infoWindow.align = "Center";

										var labelPt = geometryUtils
												.getPolygonCenterPoint(graphic.geometry);
				...
				function makePieChart(node, attributes, showFields) {
					...
					}).setTheme(CustomTheme).addPlot("default", {
						type : "Pie"
					});
					var serieValues = [];
					...
					var anim2 = new Tooltip(chart, "default", {
						text : function(o) {
							...
							return (o.chart.series[0].data[o.x].region + " "
									+ fieldName + "：" + o.y);
						}
					});
					chart.render();
