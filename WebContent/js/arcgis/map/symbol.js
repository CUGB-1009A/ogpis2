var Symbols = {
	...
	init : function() {
		require([ "esri/symbols/SimpleMarkerSymbol",
				...
				"esri/symbols/PictureMarkerSymbol", "esri/Color" ], this
				.initSymbols(this));
	},
	toArray : function(symbolType) {
	...
	initSymbols : function(thisObj) {
		return function(SimpleMarkerSymbol, SimpleLineSymbol, SimpleFillSymbol,
			...
			SimpleMarkerSymbol.STYLE_DIAMOND, 15,
					thisObj.LineSymbol.DefaultSymbol, new Color([ 255, 255, 0,
							0.5 ]));
			thisObj.PointSymbol.HlightSymbol = new SimpleMarkerSymbol(// 高亮符号
			SimpleMarkerSymbol.STYLE_DIAMOND, 15,
					thisObj.LineSymbol.DefaultSymbol, new Color([ 255, 0, 0,
							0.5 ]));
			thisObj.PointSymbol.HlightSymbol2 = new SimpleMarkerSymbol(// 高亮符号
					SimpleMarkerSymbol.STYLE_DIAMOND, 15,
							thisObj.LineSymbol.DefaultSymbol, new Color([ 55, 0, 25,
									0.5 ]));
			thisObj.PointSymbol.HlightSymbol3 = new SimpleMarkerSymbol(// 高亮符号
					SimpleMarkerSymbol.STYLE_DIAMOND, 15,
							thisObj.LineSymbol.DefaultSymbol, new Color([ 0, 264, 125,
									0.5 ]));
			// 面符号
...
function getSymbols(symbol, colors) {
	if (!symbol || !colors)
		return;
	...
	for (var i = 0; i < length; ++i) {
		...
		symbols.push($.extend(true,{},symbol));
	}
	return symbols;
}
Symbols.init();